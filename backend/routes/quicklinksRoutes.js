...
router.get('/', (_req, res) => res.json(sorted(db.data[TABLE].filter(q => q.active))));
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  await db.write();
  res.json({ id, message: 'Link criado' });
});
router.put('/:id', authMiddleware, async (req, res) => {
  const id = +req.params.id;
  const idx = db.data[TABLE].findIndex(q => q.id === id);
...
router.delete('/:id', authMiddleware, async (req, res) => {
  db.data[TABLE] = db.data[TABLE].filter(q => q.id !== +req.params.id);
  await db.write();
