...
router.get('/', (_req, res) => {
  ...
  res.json(db.data[TABLE].sort((a, b) => (a.sort_order || 0) - (b.sort_order || 0)));
});
...
router.put('/:id', authMiddleware, async (req, res) => {
  const idx = db.data[TABLE]?.findIndex((s) => s.id === Number(req.params.id));
...
router.delete('/:id', authMiddleware, async (req, res) => {
  if (!db.data[TABLE]) db.data[TABLE] = [];
  db.data[TABLE] = db.data[TABLE].filter((s) => s.id !== Number(req.params.id));
  await db.write();
